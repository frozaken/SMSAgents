...
4. Agree on a scoped topic such as `repo:smsagents/issue:12` or one supplied by the user, then call `join_topic`.
5. Send messages only when another agent can act on them. Prefer `question`, `answer`, `proposal`, `decision`, `blocker`, or `done` kinds.
...
Questions create a bounded expected-reply window. When this agent attempts to stop with an unanswered question, SMSAgents keeps a lightweight hook listener open. A reply continues the agent without a polling tool or model tokens spent waiting. Use `reply_to` when answering so the sender's listener can resolve the question.
